import argparse
import networkx
from generators import generate_circle_graph
...
def main(vertex_count, neighbors, distance, undirected, append_attributes, output_file, output_type):
	...
	if undirected:
		graph = generate_circle_graph(networkx.Graph(), vertex_count, neighbors, distance)
	else:
		graph = generate_circle_graph(networkx.DiGraph(), vertex_count, neighbors, distance)
